...
public class DefaultAdapter<T> extends RecyclerView.Adapter<DefaultViewHolder<T>> {

    public static abstract class CreateViewHolder<T> {
        @NonNull
        protected abstract DefaultViewHolder<T> onCreateViewHolder(final ViewGroup parent,
                                                                   final int viewType);
        ...
        protected void modifyViewHolder(@NonNull final T item,
                                        @NonNull final DefaultViewHolder<T> viewHolder,
    ...
    public DefaultAdapter(final @Nullable ItemClickListener<T> onClickListener,
    ...
    @Override
    public DefaultViewHolder<T> onCreateViewHolder(final ViewGroup parent, final int viewType) {
        return mCreateViewHolderListener.onCreateViewHolder(parent, viewType);
    }

    @Override
    public void onBindViewHolder(@NonNull final DefaultViewHolder<T> holder, final int position) {
    ...
    private void setClickListener(@NonNull final View view, @NonNull final T item,
                                  @NonNull final DefaultViewHolder<T> holder) {
    ...
    private static class TagWrapper<T> {
        T item;
        DefaultViewHolder<T> viewholder;

        public TagWrapper(T item, DefaultViewHolder<T> viewholder) {
            this.item = item;
    ...
    private final class OnItemClickListener implements View.OnClickListener {
        ...
        public void onClick(final View view) {
            if (mClickListener != null) {
                @SuppressWarnings ("unchecked") final TagWrapper<T> tagWrapper = (TagWrapper<T>) view.getTag();
    ...
    private final class OnItemLongClickListener implements View.OnLongClickListener {
        ...
        public boolean onLongClick(View view) {
            if (mLongClickListener != null) {
                @SuppressWarnings ("unchecked") final TagWrapper<T> tagWrapper = (TagWrapper<T>) view.getTag();
                if (tagWrapper.viewholder.getAdapterPosition() != RecyclerView.NO_POSITION) {
                    return mLongClickListener.onItemLongClick(tagWrapper.item,
                            tagWrapper.viewholder, view);
                }
            }
            return false;
        }
